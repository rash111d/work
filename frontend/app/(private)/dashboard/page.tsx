...
export default function DashboardPage() {
  ...
  const stats = [
    { label: "Мои проекты", value: dashboard.stats.projects, icon: FolderKanban, hint: "Активные" },
    { label: "Мои заявки", value: dashboard.stats.pending_applications, icon: Clock3, hint: "На рассмотрении" },
    { label: "Подходящие проекты", value: dashboard.recommended_projects.length, icon: CheckCircle2, hint: "Найдены для вас" }
  ];
      ...
      <section className="grid gap-4 lg:grid-cols-[1.4fr_0.9fr]">
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-bold">Рекомендуемые проекты</h2>
            <Link href="/projects" className="text-sm font-semibold text-brand">
              Все проекты
            </Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {dashboard.recommended_projects.length ? (
              dashboard.recommended_projects.slice(0, 6).map((project) => (
                <ProjectCard key={project.id} project={project} compact />
              ))
            ) : (
              <Card className="p-6 text-sm text-muted sm:col-span-2 xl:col-span-3">
                Подберите навыки в профиле, и здесь появятся проекты с совпадающим стеком.
              </Card>
            )}
          </div>
        </div>
